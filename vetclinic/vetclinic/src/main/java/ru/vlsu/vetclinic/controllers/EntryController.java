...
public class EntryController {
    ...
    public String showDeleteEntry(@PathVariable("id") Integer id, Model model){
        Entry entry = entryRepo.findById(id).get();
        Date date = new Date();                 //но я не уверена что это работает, надо текущую дату как то получить
        if (entry.getDate().before(date))      //надо создавать свободную дату только если она свежая
        {
